...
describe('processIdiomaticDateTime', () => {
  test.each([
    [
      '01/02/2024 12:30 -0230',
      undefined,
      [2024, 1, 2, false, 12, 30, 0, 0, -150],
    ],
    [
      '01/02/+2024 12:30 -0230',
      undefined,
    ...
    [
      '2024/01/02 12:30:10.5',
      '+0100',
      [2024, 1, 2, false, 12, 30, 10, 0.5, 60],
    ],
    ['+2024/01/02 12:30', '+0100', [2024, 1, 2, false, 12, 30, 0, 0, 60]],
    ['-2024/01/02 12:30', '+0100', [-2024, 1, 2, false, 12, 30, 0, 0, 60]],
  ...
  test('Raises an error when no time component can be found', () =>
    expect(() =>
      processIdiomaticDateTime({ name : 'foo' }, '01/02/2024')).toThrow(/does not contain a recognizable time component/))

  test('Raises an error when no date component can be found', () =>
    expect(() => processIdiomaticDateTime({ name : 'foo' }, '12:30 PM')).toThrow(
      /does not contain a recognizable date component/
    ))

  test('Raises an error when an ambiguous date component is found', () =>
    expect(() =>
      processIdiomaticDateTime({ name : 'foo' }, '1/1/1 12:30 PM')).toThrow(/is ambiguous/))
})
