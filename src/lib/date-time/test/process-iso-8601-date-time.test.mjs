...
describe('processISO8601DateTime', () => {
  test.each([
    ...
    [
      ...
      undefined,
      [2024, 1, 2, false, 12, 30, 30, 0.6, 0],
    ],
    ['2024-01-02T12:30:20Z', undefined, [2024, 1, 2, false, 12, 30, 20, 0, 0]],
    [
      '2024-01-02T12:30:20.51Z',
      undefined,
      [2024, 1, 2, false, 12, 30, 20, 0.51, 0],
    ],
    [
      '2024-01-02T12:30:20.51+0130',
      undefined,
      [2024, 1, 2, false, 12, 30, 20, 0.51, 90],
    ],
    ['2024-01-02T12:30:20.51', 'Z', [2024, 1, 2, false, 12, 30, 20, 0.51, 0]],
    [
      '2024-01-02T12:30:20.51',
      '-0200',
      [2024, 1, 2, false, 12, 30, 20, 0.51, -120],
    ],
  ])('ISO 8601 %s (tz: %s) => %p', (input, tz, result) => {
    const iso8601Match = input.match(iso8601DateTimeRe)
    expect(processISO8601DateTime({ name : 'foo' }, iso8601Match, tz)).toEqual(
      result
    )
  })
  ...
  test("Throws an error when presented with 'week of year' style date", () => {
    ...
    expect(() => processISO8601DateTime({ name : 'foo' }, iso8601Match)).toThrow(
      /week of year/
    )
  })
