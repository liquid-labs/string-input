...
describe('DateTime', () => {
  const validInput = [
    ...
    ['1/2/2024 12:30', { max : DateTime('1/2/2024 12:30', { name: 'max' }) }, [2024, 1, 2, false, 12, 30, 0, 0, undefined]],
    ['1/2/2024 12:30', { min : DateTime('1/2/2024 12:30', { name: 'min' }) }, [2024, 1, 2, false, 12, 30, 0, 0, undefined]],
  ...
  const failureInput = [
    ...
    ['1/2/2024 24:00', { noEOD : true }, "uses disallowed special EOD time '24:00'\\.$"],
    // test both input and max use local time zone if net specified
    ['1/2/2024 12:30', { max : '1/2/2024 12:29' }, "constraint 'max'.*?is out of range.*?must be less than or equal to '2024-01-02 12:29:00 [+-]\\d{4}'\\.$"],
    ['1/2/2024 12:30 Z', { max : new Date('1/2/2024 12:29 Z') }, "constraint 'max'.*?is out of range.*?must be less than or equal to '2024-01-02 12:29:00 \\+0000'\\.$"],
    ['1/2/2024 12:30 Z', { max : DateTime('1/2/2024 12:29 Z') }, "constraint 'max'.*?is out of range.*?must be less than or equal to '2024-01-02 12:29:00 \\+0000'\\.$"],
    // test both input and min will both use local time zone if not specified
    ['1/2/2024 12:30', { min : '1/2/2024 12:31' }, "constraint 'min'.*?is out of range.*?must be greater than or equal to '2024-01-02 12:31:00 [+-]\\d{4}'\\.$"],
    ['1/2/2024 12:30 Z', { min : '1/2/2024 12:31 Z' }, "constraint 'min'.*?is out of range.*?must be greater than or equal to '2024-01-02 12:31:00 \\+0000'\\.$"],
    ['1/2/2024 12:30 Z', { min : new Date('1/2/2024 12:31 Z') }, "constraint 'min'.*?is out of range.*?must be greater than or equal to '2024-01-02 12:31:00 \\+0000'\\.$"],
    ['1/2/2024 12:30 Z', { min : DateTime('1/2/2024 12:31 Z') }, "constraint 'min'.*?is out of range.*?must be greater than or equal to '2024-01-02 12:31:00 \\+0000'\\.$"],
    ['1/2/2024 12:30', { max : /invalid type/ }, "'max'.*? is nonconvertible type"],
    ['1/2/2024 12:30', { min : /invalid type/ }, "'min'.*? is nonconvertible type"],
    ...
    .concat([ // the following reference the input name differently
      ['foo', { name : 'foo' }, "'foo' with value 'foo' does not contain a recognizable time component\\.$"],
      ['1/1/2024', { name : 'foo' }, "argument 'foo' with value '1/1/2024' does not contain a recognizable time component\\.$"],
      ['2400', { name : 'foo' }, "argument 'foo' with value '2400' does not contain a recognizable date component\\.$"],
      ['1/2/2024 12:30', { name : 'foo', max : 'bar' }, "'foo' constraint 'max' with value 'bar' does not contain a recognizable time component\\.$"],
      ['1/2/2024 12:30', { name : 'foo', min : 'bar' }, "'foo' constraint 'min' with value 'bar' does not contain a recognizable time component\\.$"]
    ])
